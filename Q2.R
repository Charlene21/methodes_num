#Question a
#ModeleMerton(0.05,200,0,0.02,0,0.25,100,100)
ModeleMerton<-function(lambda, T, mu, delta, sigma, pas,S0){
  ...
  for(i in 1:length(Tn_recompose)){
    ...
    #cat("j : ", j, "\n");
    Wt = rnorm(1, mean=0, sd=sqrt(Tn_recompose[i]/T));
    #cat("Wt : ", Wt, "\n tn_rec : ", Tn_recompose, "\n x_comp : ", X_compose, "\n");
 ...
 # cat("X : ", X, "\n");
  ...
  plot(Tn_recompose,St, main="Modele de Merton", type='l'); 
